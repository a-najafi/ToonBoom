...
namespace ToonBoomEditor.Powerup
{
    ...
    [CustomPropertyDrawer(typeof(PowerupEntity), true)]
    public class PowerupEntityDrawer : PropertyDrawer
    {
        private static Type[] derivedTypes;

        static PowerupEntityDrawer()
        {
            // Find all non-abstract child classes of BaseClass
            derivedTypes = AppDomain.CurrentDomain.GetAssemblies()
                .SelectMany(a => a.GetTypes())
                .Where(t => t.IsSubclassOf(typeof(PowerupEntity)) && !t.IsAbstract)
                .ToArray();
        }
